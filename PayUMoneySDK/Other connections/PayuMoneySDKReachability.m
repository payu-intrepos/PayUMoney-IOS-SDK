...
@implementation PayuMoneySDKReachability
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info)
{
	#pragma unused (target, flags)
	NSCAssert(info != NULL, @"info was NULL in ReachabilityCallback");
	NSCAssert([(NSObject*) info isKindOfClass: [PayuMoneySDKReachability class]], @"info was wrong class in ReachabilityCallback");

	//We're on the main RunLoop, so an NSAutoreleasePool is not necessary, but is added defensively
	// in case someon uses the Reachablity object in a different thread.
	NSAutoreleasePool* myPool = [[NSAutoreleasePool alloc] init];
	
	PayuMoneySDKReachability* noteObject = (PayuMoneySDKReachability*) info;
    
    //posting Notification in Main Queue
...
{
	PayuMoneySDKReachability* retVal = NULL;
...
+ (PayuMoneySDKReachability*) reachabilityForLocalWiFi;
{
	...
	PayuMoneySDKReachability* retVal = [self reachabilityWithAddress: &localWifiAddress];
	if(retVal!= NULL)
	{
